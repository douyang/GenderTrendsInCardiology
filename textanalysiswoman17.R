...
ayo1<-read.csv("spine17ESJ.txt",sep='\t',stringsAsFactors=FALSE)
ayo2<-read.csv("spine17JNS.txt",sep='\t',stringsAsFactors=FALSE)
ayo3<-read.csv("spine17JSD.txt",sep='\t',stringsAsFactors=FALSE)
ayo4<-read.csv("spine17.txt",sep='\t',stringsAsFactors=FALSE)
ayo5<-read.csv("spine17J.txt",sep='\t',stringsAsFactors=FALSE)
ayo6<-read.csv("spine17CSS.txt",sep='\t',stringsAsFactors=FALSE)


ayo<-rbind(ayo1,ayo2,ayo3,ayo4,ayo5)
write.csv(ayo,"allspines17.csv")
ayo<-read.csv('allspines17.csv',stringsAsFactors=FALSE)
setwd('D:\\orthoresearch\\scrape\\woman\\17')
#ayo<-read.csv("C:\\Users\\dsing_000\\Desktop\\orthoresearch\\scrape\\allspines16.csv",stringsAsFactors=FALSE)
ayo$Type1<-tolower(ayo$Type1)
ayo$Type2<-tolower(ayo$Type2)
ayo$Type3<-tolower(ayo$Type3)
ayo$Type<-ayo[c("Type1", "Type2", "Type3")]

ayo$article<-apply(ayo$Type,1, function(c) (sum (c %in% "journal article"))>0)
ayo<-ayo[ayo$article==TRUE,]

library(data.table)
ayo$ann<-ayo$Title %like% "Announcements"
ayo<-ayo[ayo$ann==FALSE,]
ayo<-ayo[!is.na(ayo$numAuthor),]

ayo$ym<-paste(ayo$Year,ayo$Month)

#ayo<-ayo[ayo$Abstract!="",]
#ayo$AbstractC<-ayo$Abstract

#ayo$Abstract<-tolower(ayo$Abstract)

#abstract2015<-data.frame(ayo$texts[ayo$Yeargroup=="14-15"])
#colnames(abstract2015)<-"texts"
#abstract2015$cervical<-"lumbar" %in% abstract2015$texts


ayo$J<-""
ayo$J[ayo$Journal=="European spine journal : official publication of the European Spine Society, the European Spinal Deformity Society, and the European Section of the Cervical Spine Research Society"]<-"ESJ"
ayo$J[ayo$Journal=="The spine journal : official journal of the North American Spine Society"]<-"SpineJ"
ayo$J[ayo$Journal=="Journal of spinal disorders &amp; techniques"]<-"JSD"
ayo$J[ayo$Journal=="Clinical spine surgery"]<-"JSD"
ayo$J[ayo$Journal=="Journal of neurosurgery. Spine"]<-"JNS"
ayo$J[ayo$Journal=="Spine"]<-"Spine"


ayo$Journal1<-ayo$Journal
ayo$Journal1[ayo$Journal=="Journal of spinal disorders &amp; techniques"|ayo$Journal=="Clinical spine surgery"]<-"Journal of Spinal Disorders & Techniques / Clinical Spine Surgery"
ayo$Journal1[ayo$Journal=="European spine journal : official publication of the European Spine Society, the European Spinal Deformity Society, and the European Section of the Cervical Spine Research Society"]<-"European Spine Journal"
ayo$Journal1[ayo$Journal=="Journal of neurosurgery. Spine"]<-"Journal of Neurosurgery: Spine"
ayo$Journal1[ayo$Journal=="The spine journal : official journal of the North American Spine Society"]<-"The Spine Journal"


ayo$ref<-paste(ayo$firstAuthorL," et al. \"",ayo$Title,'\" ',ayo$Journal1," (",ayo$Year,")",sep="")
...
#ayo<-ayo[tolower(ayo$Country)=="united states",]
#ayo<-ayo[!is.na(ayo$Country),]

#ayo$faF<-unlist(lapply(strsplit(ayo$firstAuthorF," "), '[[',1))
#ayo<-merge(ayo,nameMF,by.x="faF",by.y="name",all.x=TRUE)
...
independence_test(c(0,0,6.66666666666667,0,0,0,6.25,2.85714285714286,5.26315789473684,2.5,4.34782608695652,3.84615384615385,7.14285714285714,3.7037037037037,7.69230769230769,3.40909090909091,10.0719424460432,5.79710144927536,7.75193798449612,10.4838709677419,13.5714285714286,9.16666666666667,12.8654970760234,12.9213483146067,16.0220994475138,12.664907651715,16.0887656033287,13.1322957198444,16.9187145557656,16.2983425414365,16.8932038834951,14.0221402214022,15.8423493044822,18.7299035369775,16.9601482854495,17.7617328519856,20.6896551724138,20.01982160555)
~c(1978:2015),teststat="quad")

independence_test(c(0,0,0,0,0,0,0,5,0,0,0,4.8780487804878,2.04081632653061,4.61538461538462,13.7931034482759,1.19047619047619,4.58715596330275,8.8,4.03225806451613,4.13223140495868,10.9090909090909,7.14285714285714,12.6666666666667,9.39226519337017,11.6935483870968,10.6824925816024,11.9426751592357,9.88184747583244,12.3,10.4058272632674,13.7380191693291,12.906610703043,12.6796280642434,13.338997451147,11.1338100102145,11.2341772151899,9.80215827338129,10.6666666666667)
                  ~c(1978:2015),teststat="quad")
